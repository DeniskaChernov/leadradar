...
## BUGS
1. Concurrency race in message editing across multiple notifier workers could attempt duplicate edits.
2. Missing scheduled morning/evening digest command handlers and cron dispatches.
...
## PROPOSED FIX
1. Harden `TelegramLeadNotifier` with atomic single-claim edit locking.
2. Implement `TelegramDigestService` generating rich morning/evening summaries.
3. Enhance inline keyboard callbacks for immediate triage.
