# Lead Radar — offline hardening

Lead Radar — локальный Mini App + Telegram-бот для мониторинга публичного спроса у мебельных
конкурентов. Система сохраняет найденного человека в своей базе, показывает откуда он пришёл,
анализирует комментарий, ведёт лида по воронке и запоминает результат сделки.

Текущий статус: **не готов к live pilot**. Внешние вызовы выключены; актуальная программа
завершения находится в `ROADMAP.md`, краткое машинное состояние — в `State.md`.

## Что уже реализовано

Основной интерфейс теперь не Telegram-команды, а понятный русскоязычный Mini App / CRM:

- **Обзор** — что происходит прямо сейчас, сколько клиентов, сигналов, HOT-лидов, задач и продаж;
- **Радар** — каждый сохранённый комментарий с источником, Reel, оценкой и объяснением;
- **Лиды** — рабочая Kanban-воронка от нового интереса до продажи;
- **Клиенты** — поиск по username/имени и единая история человека;
- **Аудитории** — управляемый реестр из 28 определений, 20 доказуемых активных аудиторий,
  facets и campaign briefs;
- **Карточка клиента 360°** — сигналы, заметки, квалификация, задачи, лиды, сделки и история действий;
- **Задачи** — кому и когда нужно написать/позвонить;
- **Сделки** — сумма, товар, результат и причина проигрыша;
- **Аналитика** — источники лидов, намерения, товары, продажи и конверсия;
- **Конкуренты** — карта рынка, коммерческий спрос, эффективность Reels, пересечения покупателей и возможности;
- **Развитие** — текущая стадия проекта, следующие этапы и источники будущего роста лидов;
- **Система** — состояние интеграций, история проверок и расход внешних запросов.

Telegram остаётся быстрым каналом сигналов и действий менеджера.

Интерфейс адаптирован для desktop и Telegram Mini App: нижняя навигация сохраняет доступ ко всем
11 разделам, фильтры имеют доступные названия, а длинные справочные списки сворачиваются без потери
данных. Штатная остановка сервиса выполняется без аварийного traceback.

## Audience Engine

Lead Radar пересчитывает коммерческий профиль контакта из уже сохранённых публичных сигналов:
стадию покупки, активность, ценность, товарные интересы, намерения и пересечение конкурентов.
Контакты автоматически входят и выходят из активных определений реестра — HOT-окна,
товарные категории, цена, наличие, доставка, B2B / HoReCa и мультиконкурентный спрос.

Экран аудитории показывает состав сегмента, наблюдаемые темы и готовый campaign brief без вызова
OpenAI. Это не скрытый профиль человека: используются только публичные сигналы, доступные системе.
Один Instagram username не разрешает экспорт. Eligibility появляется только после того, как
менеджер получил first-party телефон в реальном общении и зафиксировал квалификацию в CRM.

## Уведомления об усилении интереса

После каждого нового обработанного сигнала Lead Radar сравнивает новый коммерческий профиль с
предыдущим. Менеджер получает отдельное уведомление «Лид стал горячее» только когда изменение
помогает принять решение: человек появился у нового конкурента, указал значимое количество,
перешёл в B2B/HOT/high-value, проявил новый сильный intent, вернулся после паузы или заметно
продвинулся по стадии покупки.

Несколько причин одного сигнала объединяются в одно событие. Событие и адресат уведомления имеют
уникальные ключи в базе, поэтому retry и повторная обработка не создают дублей. Мелкие колебания
score остаются в технической истории и не засоряют Telegram.

## Signal First

Новый уникальный комментарий сначала сохраняется в базе вместе с `PublicSignal` и неизменяемым
событием. После commit менеджер получает короткое уведомление «Новый сигнал», а локальный/AI-анализ
выполняется следом. Готовый результат обновляет то же Telegram-сообщение. Если Telegram не разрешает
редактирование, система отправляет один короткий follow-up и больше его не дублирует.

Режим по умолчанию — **каждый новый комментарий**. Также доступны «только покупательский интерес»
и «только горячие лиды», включая отдельную настройку для конкурента. Baseline-история и replay/mock
никогда не отправляют production-уведомления.

Очередь Telegram-уведомлений обслуживается независимо от Instagram-поиска. Если лид назначен,
карточка идёт этому менеджеру; иначе — настроенным администраторам. Повтор одной карточки одному
адресату блокируется на уровне базы.


## Карта рынка V3.2

Lead Radar больше не ограничен AIKO. При запуске система бесплатно синхронизирует встроенную карту
рынка в своей БД:

- **16 подтверждённых Instagram-конкурентов** находятся непосредственно в радаре;
- **24 неперенесённых market candidates** сохранены для дальнейшей проверки;
- новые подтверждённые аккаунты стартуют **на паузе**, поэтому каталог не увеличивает расход API;
- в Mini App есть сегменты, Tier A/B/C, HOT-rate, продажи, выручка и рекомендация по приоритету;
- кандидата можно подтвердить и добавить в мониторинг прямо из раздела «Конкуренты».

Мультиконкурентный радар реализован как offline-фундамент. Schema, security, workflow integrity,
Catalog → Offer → Demand Gap и Unit Economics закрыты; текущая работа — независимые quality gates,
затем Agent/MCP и deployment readiness. Полный порядок находится в `ROADMAP.md`.

## Competitor Intelligence V2

Раздел «Конкуренты» строит аналитику только по уже сохранённым публичным комментариям. Для каждой
компании доступны доля коммерческих сигналов, уникальные покупатели, HOT-rate, вопросы о цене,
наличии, доставке и количестве, а также эффективность публикаций на 100 наблюдаемых комментариев.

Сеть пересечений показывает контакты, замеченные у нескольких компаний, причём повторные сигналы
у одного конкурента не увеличивают связь. Рекомендации формируются детерминированно и не расходуют
OpenAI-токены. Lead Radar не видит Direct и при текущем provider не делает выводов о том, ответил ли
конкурент публично.

## Самое важное: безопасная разработка без расхода токенов

V3.2 по умолчанию работает fail-closed. Для обычной разработки не нужны ScrapeCreators,
Bright Data или OpenAI.

Безопасные значения уже находятся в `.env.example`:

```dotenv
EXTERNAL_LIVE_UNLOCK=
INSTAGRAM_PROVIDER=replay
INSTAGRAM_LIVE_CALLS_ENABLED=false
OPENAI_LIVE_CALLS_ENABLED=false
AI_MODE=hybrid
AI_PENDING_RETRY_ENABLED=false
```

Даже если случайно поставить `*_LIVE_CALLS_ENABLED=true`, внешний запрос **не уйдёт**, пока не
будет добавлен второй предохранитель:

```dotenv
EXTERNAL_LIVE_UNLOCK=ALLOW_EXTERNAL_CALLS
```

Для Instagram дополнительно действуют:

```dotenv
INSTAGRAM_DAILY_REQUEST_LIMIT=100
INSTAGRAM_MAX_UNITS_PER_SCAN=8
INSTAGRAM_MANUAL_LIVE_SCAN_ONLY=true
MONITOR_SCHEDULE_ENABLED=false
```

То есть live-проверка имеет дневной предел, предел одного запуска и требует отдельного
подтверждения в интерфейсе/Telegram.

## Локальный AI без OpenAI

Очевидные сигналы классифицируются бесплатно на компьютере:

- `цена?`, `сколько стоит?`;
- `narxi?`, `qancha?`, `nech pul?`;
- `Нархи?`, `Нархи қанча?`, `Нархини ёзинг`;
- `bormi?`, `борми?`, `мавжуд`;
- `доставка`, `yetkazib berish`, `етказиб бериш`;
- конкретное количество;
- `+` — всегда сигнал для OpenAI: смысл определяется по описанию Reel (CTA на плюс, мебель и т.д.).
- Запросы цены по чужим категориям (часы, телефоны, авто и т.п.) локально отсекаются как «не наш ассортимент».
- Ценовые маркеры без контекста мебели в Reel уходят в OpenAI, а не становятся лидом по умолчанию.

Похвала, эмодзи и обычные реакции также отсекаются локально.

Если локальные правила не уверены, комментарий получает статус **«Нужна AI-проверка»**. Он не
объявляется автоматически плохим лидом и не расходует OpenAI без явного разрешения.

Кнопка **«Разобрать сохранённые сигналы»** в Mini App всегда локальная: она технически не имеет
доступа к OpenAI даже если live-AI позже включён.

## Глубокий разбор лида

Для каждого нового результата система сохраняет не только `lead_score` и intent, но и:

- уверенность классификации;
- стадию готовности к покупке;
- срочность и предполагаемый горизонт решения;
- наблюдаемые доказательства и признаки неопределённости;
- одно конкретное следующее действие менеджера.

OpenAI используется только для неоднозначных случаев в разрешённом hybrid/live-режиме. Ответ
валидируется через Structured Outputs, не сохраняется на стороне Responses API (`store=false`) и
кешируется локально. Повторный одинаковый контекст не создаёт новый внешний запрос.

## Уже сохранённые данные Aiko в этой сборке

Копия базы, вошедшая в V3.2, мигрирована на последнюю схему и локально разобрана без внешних API:

- 25 клиентов;
- 28 публичных комментариев;
- 8 HOT-лидов;
- 17 реакций / не лидов;
- 3 неоднозначных сигнала, оставленных на AI-проверку;
- 0 внешних AI/Instagram операций во время этого разбора.

Исторические лиды не рассылаются пачкой в Telegram.

## Быстрый запуск на Windows

### 1. Первый запуск

Распакуйте архив и запустите:

```text
setup_windows.bat
```

Скрипт:

1. создаст `.venv`;
2. установит зависимости;
3. создаст `.env`, если его нет;
4. сделает резервную копию SQLite;
5. применит Alembic migrations;
6. проверит целостность базы.

### 2. Безопасный локальный интерфейс без поиска лидов

```text
start_web_only_windows.bat
```

При `LEAD_SEARCH_ENABLED=false` ручные, CLI и фоновые циклы поиска заблокированы. Вместе с
нулевыми лимитами и пустым `EXTERNAL_LIVE_UNLOCK` это исключает расходы Instagram/OpenAI.

Откройте:

```text
http://127.0.0.1:8000
```

### 3. Работа с основной локальной базой

```text
start_web_only_windows.bat
```

или, если настроен Telegram:

```text
start_windows.bat
```

## Replay Mode

Replay — главный режим разработки. Он воспроизводит реалистичный Instagram-сценарий из локального
fixture и не делает сетевых запросов.

В разделе **Система** можно переключать шаги replay-сценария и затем нажимать **Проверить сейчас**.
Так можно многократно тестировать цепочку:

```text
Reel → комментарий → Contact → Lead → CRM → сделка
```

без расхода credits.

## Анализ сохранённой истории

Только локальные правила, 0 OpenAI:

```powershell
python -m scripts.analyze_history --limit 100
```

Неоднозначные сигналы останутся в `AI_PENDING`.

OpenAI можно разрешить только осознанно:

```powershell
python -m scripts.analyze_history --limit 100 --allow-openai
```

Но даже эта команда не отправит запрос, пока одновременно не установлены:

```dotenv
OPENAI_LIVE_CALLS_ENABLED=true
EXTERNAL_LIVE_UNLOCK=ALLOW_EXTERNAL_CALLS
```

## Интеграции

Безопасная проверка конфигурации и БД, без Instagram API:

```powershell
python -m scripts.check_integrations
```

Минимальная live-проверка выполняется только явно:

```powershell
python -m scripts.check_integrations --live
```

Instagram live всё равно подчиняется двойному предохранителю. Фоновое расписание также отключено по умолчанию (`MONITOR_SCHEDULE_ENABLED=false`) и включается отдельно после ручного пилота.

## Целостность базы

```powershell
python -m scripts.check_data_integrity
```

Проверяются в том числе дубли comment ID, post URL, lead/comment, deal/lead и Telegram notifications.

## Автоматические тесты

```powershell
python -m ruff check .
```

Полный offline gate обязателен перед завершением каждого существенного этапа:

```powershell
python -m pytest -q
python -m ruff check .
python -m compileall -q app alembic tests
python -m alembic check
python -m scripts.check_data_integrity
```

Browser E2E (Chromium) входит в `pytest`. Один раз после установки зависимостей:

```powershell
python -m playwright install chromium
```

Контрольная точка 2026-08-30: 238 тестов, Ruff, compileall, integrity и `alembic check`
проходят. Stages 0–5 закрыты; активный этап — Independent quality gates.

## Основные переменные `.env`

```dotenv
TELEGRAM_BOT_TOKEN=
TELEGRAM_ADMIN_CHAT_IDS=
TELEGRAM_MANAGER_CHAT_IDS=
TELEGRAM_VIEWER_CHAT_IDS=

EXTERNAL_LIVE_UNLOCK=

DATABASE_URL=sqlite+aiosqlite:///./lead_radar.db

AI_MODE=hybrid
OPENAI_API_KEY=
OPENAI_MODEL=gpt-5-mini
OPENAI_LIVE_CALLS_ENABLED=false
OPENAI_DAILY_REQUEST_LIMIT=25
AI_PENDING_RETRY_ENABLED=false

INSTAGRAM_PROVIDER=replay
INSTAGRAM_LIVE_CALLS_ENABLED=false
INSTAGRAM_DAILY_REQUEST_LIMIT=100
INSTAGRAM_MAX_UNITS_PER_SCAN=8
INSTAGRAM_MANUAL_LIVE_SCAN_ONLY=true

SCRAPECREATORS_API_KEY=
BRIGHTDATA_API_KEY=

# `COMPETITORS` теперь только bootstrap. Полная карта рынка живёт в БД.
COMPETITORS=aiko.uz
HOT_LEAD_THRESHOLD=70
PROCESS_EXISTING_COMMENTS=false
ANALYZE_BASELINE_COMMENTS=true
HISTORICAL_ANALYSIS_BATCH_SIZE=25
LEAD_SEARCH_ENABLED=false

WEB_ENABLED=true
WEB_HOST=127.0.0.1
WEB_PORT=8000
WEB_PUBLIC_URL=
WEB_AUTH_ENABLED=false
TELEGRAM_INIT_DATA_MAX_AGE_SECONDS=300
```

## Когда будем включать реальный Instagram

Не включайте live во время разработки интерфейса. Правильная последовательность:

1. закрыть stages 0–9 из `ROADMAP.md`;
2. получить чистые schema, security, offline quality и deployment gates;
3. включить `LEAD_SEARCH_ENABLED=true` только для replay-проверки;
4. проверить Mini App, CRM, backup/restore и целостность базы;
5. отдельно согласовать provider, бюджет и controlled live pilot;
6. выполнить один подтверждённый live scan и сразу вернуть live-флаги в disabled.

Для реального Instagram понадобятся одновременно:

```dotenv
INSTAGRAM_PROVIDER=scrapecreators
INSTAGRAM_LIVE_CALLS_ENABLED=true
EXTERNAL_LIVE_UNLOCK=ALLOW_EXTERNAL_CALLS
```

После контрольного теста `EXTERNAL_LIVE_UNLOCK` лучше снова очистить.

## Telegram Mini App и Railway

Локально интерфейс работает как обычный web app. После Railway:

1. SQLite заменяем на PostgreSQL через `DATABASE_URL`;
2. получаем HTTPS URL;
3. задаём `WEB_PUBLIC_URL`;
4. задаём непересекающиеся списки `TELEGRAM_ADMIN_CHAT_IDS`,
   `TELEGRAM_MANAGER_CHAT_IDS` и `TELEGRAM_VIEWER_CHAT_IDS`;
5. включаем `WEB_AUTH_ENABLED=true`;
6. Mini App проверяет свежий Telegram `initData` на сервере;
7. mutating requests требуют подписанную session cookie, CSRF token и достаточную роль.

Приложение fail-closed: публичный host/URL без HTTPS и серверной Telegram-авторизации
отклоняется при чтении конфигурации.

Полный контракт ролей, сессии, CSRF и security headers: `docs/WEB_SECURITY_BOUNDARY.md`.

## Privacy

Lead Radar использует только публичную информацию: username, display name, публичный комментарий,
публичную ссылку профиля, Reel URL/caption и timestamps. Скрытые телефоны, email, private messages
и данные из утечек система не получает.

Подробная архитектура находится в `ARCHITECTURE.md`, текущее состояние — в `PROJECT_STATUS.md`,
дорожная карта — в `ROADMAP.md`.
