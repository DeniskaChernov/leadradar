...
## V4.1.1 · system and interface audit

- исправлен неверный переход KPI «Нужно связаться»;
- нижняя мобильная навигация больше не переносит одиннадцатый пункт за пределы панели;
- выровнена сетка KPI, восстановлен основной цветовой token и свёрнут длинный список кандидатов;
- все фильтры и рабочие текстовые поля получили доступные названия;
- штатная остановка сервиса больше не выводит traceback `KeyboardInterrupt`;
- 14 основных экранов проверены без overflow, duplicate id и ошибок browser console;
- 77 тестов, Ruff, compileall, Alembic и integrity scan прошли без внешних вызовов.

## V4.1 Foundation · Competitor Intelligence V2

- добавлены коммерческий intent rate, уникальные покупатели и тематическая структура спроса;
- отдельная карточка конкурента ранжирует публикации по коммерческим сигналам на 100 комментариев;
- overlap network идемпотентно считает контакты между парами компаний без раздувания дублями;
- возможности и следующие действия выводятся детерминированно из наблюдаемой базы;
- интерфейс честно сообщает, что Direct и ненаблюдаемые ответы конкурента неизвестны;
- 75 тестов, Ruff, compileall, integrity scan и browser smoke прошли без внешних вызовов.
